...
export default function ProductModal({
  ...
  const handleCategoryChange = (selectedOption: any) => {
    const catId = Number(selectedOption?.value);

    setValue("category", catId);
    setValue("subcategory", null); // reset

    const selectedCat = data?.data?.find(
      (cat: any) => Number(cat?.id) === catId
    );
  ...
  useEffect(() => {
    ...
    const selectedCat = data?.data?.find(
      (cat: any) => Number(cat?.id) === Number(productForm?.category)
    );
        ...
        <div className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:p-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-4xl mx-auto">
            ...
            <div className='grid grid-cols-1 gap-2'>
              ...
              <div className='col-span-12 sm:col-span-6 md:col-span-6 lg:col-span-6'>
                <Input required label="Weight (Kg)" type="number" {...register('weight')} />
                {errors.weight && (
